...
<script>
  (function () {
    ...
    async function typePostLine(text, speed) {
      ...
      await typeInto(line, text, speed || 24);
      return line;
    }

    async function runMemoryTest() {
      const line = addPostLine();
      await typeInto(line, 'Memory Test: ', 24);
      ...
      await wait(650);
      await typeInto(line, ' OK', 100);
    }

    async function typeDeviceCheck(prefix, result) {
      const line = addPostLine();
      await typeInto(line, prefix, 18);
    ...
    async function startBoot() {
      ...
      await typePostLine('VERONICA V-486 BIOS v1.04', 25);
      await typePostLine('Copyright (C) 1998-2026 Veronica Pletsch', 23);
      await typePostLine('', 1);
      await typePostLine('VP-486 CPU at 2500 MHz, 4 Processor(s)', 24);
      await runMemoryTest();
      await wait(900);
      await typePostLine('', 1);
      await typePostLine('Portfolio Plug and Play ROM Extension v1.0.4', 18);
      ...
      await typePostLine('Booting from C:\\', 25);
      await wait(1000);
