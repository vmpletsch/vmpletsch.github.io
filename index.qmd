...
<style>
  ...
  .alt-window-task.is-about-task, .alt-window-task.is-about-task.is-active { align-self: stretch; background: transparent; border: 0; box-sizing: border-box; flex: 0 0 120px; max-width: 120px; min-width: 120px; padding: 0; }
  .alt-window-task.is-about-task img { display: block; height: 38px; image-rendering: pixelated; width: 120px; }
...
<script>
  (function () {
    ...
    function updateWindowTaskArtwork(entry, isOpen) {
      if (!entry || !entry.task.classList.contains('is-about-task')) return;
    ...
    function openPortfolioWindow(link) {
      ...
      if (title === 'About Me') {
        task.classList.add('is-about-task');
        task.dataset.openArtwork = 'images/ux-dashboard/aboutme_open.png';
        task.dataset.closedArtwork = 'images/ux-dashboard/aboutme_closed.png';
        task.textContent = '';
        const taskArtwork = document.createElement('img');
        taskArtwork.src = task.dataset.openArtwork;
        taskArtwork.alt = '';
        task.append(taskArtwork);
      }
